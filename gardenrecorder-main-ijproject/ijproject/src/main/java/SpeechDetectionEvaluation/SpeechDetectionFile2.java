        ...
        try {
            ...
            dispatcher.addAudioProcessor(new PitchProcessor(
                    ...
                    (pitchDetectionResult, audioEvent) -> {
                        detectedPitch = pitchDetectionResult.getPitch();
                        if (detectedPitch > 600) {
                            isSpeech = false;
                        }
                    }
            ));
            ...
            dispatcher.addAudioProcessor(new AudioProcessor() {
                @Override
                public boolean process(AudioEvent audioEvent) {
                    System.out.println(isSpeech && previousIsSpeech ? "Speech detected: yes, Pitch: " + detectedPitch + " Hz" : "Speech detected: no, Pitch: " + detectedPitch + " Hz");
                    if (isSpeech) {
                        amountYes++;
                    } else {
                        amountNo++;
